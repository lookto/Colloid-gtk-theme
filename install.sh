      ...
      for variant in $@; do
        case "$variant" in
          ...
          kanagawa)
            kanagawa="true"
            ctype="-Kanagawa"
            echo -e "Kanagawa ColorSheme version! ..."
            shift
            ;;
          black)
            blackness="true"
...
kanagawa_color() {
  sed -i "/\@import/s/color-palette-default/color-palette-kanagawa/" ${SRC_DIR}/sass/_tweaks-temp.scss
  sed -i "/\$colorscheme:/s/default/kanagawa/" ${SRC_DIR}/sass/_tweaks-temp.scss
}
